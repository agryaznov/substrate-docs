...
In short, a candle auction is basically an English auction for which end time is not known a priori and is to be determined randomly. There weren't many tamed sources of randomness back then in 17-18th centuries, when auctions of this type were popular, and people used a 🕯️ candle for that. Auction would start once a candle was lit and end once its flame expired, hence the name.

The benefit of an auction run by a candle is that it encourages participants to place their true bids from the very beggining of the auction, as there won't be a chance to perform sniping, effectively favouring the bidder who truly values the slot most. What makes it even more useful for running on a blockhain, is that such an auction mechanics prevents front-running and neutralizes possible harmful effects of smart contracts being present among bidders (see [this blog post](https://polkadot.network/blog/research-update-the-case-for-candle-auctions/) for a detailed explanation).
...
Basic logic is the following. Auction timing is split into two periods (or *phases*): _Opening_ and _Ending_.
...
In the example above, blocks `6`, `7`, `8`, `...`, `11`, `12` correspond to bidding rounds `1`, `2`, `3`, `...`, `6`, `7`.  If it turns out that there is no bids in these rounds at all, then the top bid of the whole *Opening* period wins. We can therefore say that the *Opening* period is effectively round `0`. 

## Auction Subject

Auctions are not run just for the sake of running auctions, though - there should be a prize for it. In Polkadot's and Kusama's candle auctions the prizes are parachain lease slots.  We design our candle auction smart contract in such a way as to enable various prizes, depending on the particular use case needs. In this workshop, we will setup auctions for an **_NFTs collection_** and for a **_domain name_**.

To make all the process run trustlessly on chain, the auction subjects should be some entities recognised on-chain. This means that every prize requires a separate contract to represent its on-chain logic. That brings us to cross-contract method invocation. 

### Cross-Contract Communication

There are two patterns of cross-contract calls in *ink!*: 

1. [`ink_env::invoke_contract()`](https://paritytech.github.io/ink/ink_env/fn.invoke_contract.html) function, which is a low-level way to do this, and requires explicit [selector](https://paritytech.github.io/ink-docs/macros-attributes/selector/#controlling-the-messages-selector) specification of the message being called.
2. [*ink-as-a-dependency*](https://paritytech.github.io/ink-docs/basics/cross-contract-calling) way implies taking sources of the contract being called and including them as a dependency into your contract's code.


We use the first pattern, which makes candle auction smart contract *loosely coupled* with external contracts representing auction subjects: instead of depending on their source, it relies just on their interfaces, aka *ABI*s. 

We'll consider the exact piece of code implementing such cross-contract method calling [below](#upgrade-auction-contract). Let's now move on to the overall process in which such a calling would be made, starting with an *NFT* auction flow. 

### *ERC721*

For *NFTs* we'll use a [slightly modified](https://github.com/agryaznov/ink/blob/candle-auction/examples/erc721/lib.rs#L168) *ERC721* contract example taken from the [`ink!` repository](https://github.com/paritytech/ink/blob/v3.0.0-rc7/examples/erc721/lib.rs). 
...
<Message
  ...
  The largest NFT marketplaces use this standard to trade artworks on it. If you've heard about million-dollar cyberpunk or monkey pic sold as an NFT, then you should know that under the hood it's a token minted with an *ERC721* (or sometimes its descedant *ERC1155*) contract.
...
+ Before the Auction starts, he invokes [`transfer()`](https://github.com/paritytech/ink/blob/v3.0.0-rc7/examples/erc721/lib.rs#L187) to transfer the tokens he'd like to sell to the *Auction* contract;
...
Now that Ferdie has got some NFTs to sell, it's time to set up an auction for them!
...
Newcomer (such as Bob) can now see that the winning bid is Alice's *100 Units* and that auction is still ongoing (`EndingPeriod: 14` status means that it is in 14th block of the *Ending Period*):   
...
As last block of the *Ending Period* passes by, the auction is closed: no more bids are accepted! But that's not the end yet, as we need to "blow that candle", i.e. randomly determine the decisive block and the winner.  Fot that, we have to wait until chain randomness source *matures*.
...
It doesn't matter who calls it, could be any account that is ready to pay for gas required for that transaction. Let's imagine *Ferdie* can't wait to get his money for sold NFTs so he invokes it: 
...
Once auction ended, every participant can claim their payout. 

#### Winner

Next, let's focus on the winner. Bob invokes `payout()` method:
...
The other token with `Id=5` could be transferred later to whatever address Bob chooses, e.g. as a gift to a friend. 
...
The other possible auction prize could be a domain name. Imagine you've got a nice domain name and you'd like to put it on sale without involving third parties like greedy domain registrar companies.  Hopefully, in a future Internet this will work the following way: there will be automated, transparent registrars in form of smart contracts, and domain registering and ownership trasferral operations will be processed through them. (*See [ENS](https://docs.ens.domains/) as a prominent example of such a system*).
...
1. Read and understand its [source code](https://github.com/agryaznov/candle-auction-ink). Or, at least, take its source code from some secured place (like Github) where the community has audited it.  
And,  
