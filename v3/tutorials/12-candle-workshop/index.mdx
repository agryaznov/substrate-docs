...
To make all the process run trustlessly on chain, the auction subjects should be some entities living and being managed fully on chain. This means that every subject implies a separate contract to represent its logic on chain. Here we come to cross-contract method invocation. 
...
We'll consider the exact piece of code implementing such cross-contract method calling [below](#upgrade-auction-contract). Let's now move on to overall process in which such a calling would be made, starting with a *NFT* auction flow. 
